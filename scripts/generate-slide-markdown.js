...
const { listDeckDirectories } = require('./export-pdf-lib');
...
const decks = listDeckDirectories(path.join(__dirname, '..'));
...
for (const deckDir of decks) {
  ...
  // Single-file decks are already portable Markdown documents.
  if (fs.existsSync(contentPath)) {
    const source = fs.readFileSync(contentPath, 'utf-8');
    const slideCount = (source.match(/\norder:\s*\d+\n/g) || []).length;
    fs.writeFileSync(outPath, source, 'utf-8');
    console.log(`Generated: ${outPath} (${slideCount} slides)`);
    continue;
  }
